...
export default function GameUI({
  ...
  function loadLocationFunc() {
    function afterAd() {
      ...
      if (Date.now() - loadTime > 600000) {
        ...
        console.log(
          "Not showing discord modal, waiting for " +
            (600000 - (Date.now() - loadTime)) +
            "ms"
        );
    }
    ...
    if (window.location.search.includes("learn=true")) {
      ...
      fetch(
        window.cConfig.apiUrl +
          "/api/clues/getClue" +
          (latLong ? `?lat=${latLong.lat}&lng=${latLong.long}` : "")
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.error) {
        ...
        if (val === 0) {
          setOnboarding((prev) => {
            return {
              ...
              nextRoundTime:
                Date.now() +
                (window.location.search.includes("crazygames") ? 60000 : 20000),
            };
          });
  ...
  function guess() {
    ...
    if (gameOptions.location === "all" && pinPoint) {
      ...
      findCountry({ lat: pinPoint.lat, lon: pinPoint.lng })
        .then((country) => {
          afterGuess(country);
        })
        .catch((e) => {
...
*/}
      ...
      {singlePlayerRound?.done && (
        <RoundOverScreen
          points={singlePlayerRound.locations.reduce(
          ...
          button1Press={() => {
            window.crazyMidgame(() => loadLocationFunc());
          }}
        />
      )}
          ...
          <>
            <div
              ...
              className={`miniMap ${miniMapExpanded ? "mapExpanded" : ""} ${
                (miniMapShown || showAnswer) &&
                !singlePlayerRound?.done &&
                ((!showPanoOnResult && showAnswer) || !showAnswer)
                  ? "shown"
                  : ""
              } ${showAnswer ? "answerShown" : "answerNotShown"} ${
                miniMapFullscreen && miniMapExpanded ? "fullscreen" : ""
              }`}
            >
              {!showAnswer && (
                <div
              ...
              <div
                className={`miniMap__btns ${
                  showAnswer ? "answerShownBtns" : ""
                }`}
              >
                <button
                ...
                {!multiplayerState?.inGame && (
                  <button
                    className={`miniMap__btn hintBtn ${
                      hintShown ? "hintShown" : ""
                    }`}
                    onClick={showHint}
                  >
            ...
            <div
              className={`mobile_minimap__btns ${
                miniMapShown ? "miniMapShown" : ""
              } ${
                showAnswer || singlePlayerRound?.done ? "answerShownBtns" : ""
              }`}
            >
              {miniMapShown && (
                <>
              ...
              <button
                className={`gameBtn ${
                  miniMapShown ? "mobileMiniMapExpandedToggle" : ""
                }`}
                onClick={() => {
      ...
      <span
        className={`timer duel ${!multiplayerTimerShown ? "" : "shown"}  ${
          multiplayerState?.gameData?.public ? "duel" : ""
        }`}
      >
      ...
      {singlePlayerRound && !singlePlayerRound?.done && (
        <span className="timer shown">
          {text("round", {
            r: singlePlayerRound.round,
            mr: singlePlayerRound.totalRounds,
          })}{" "}
          -{" "}
          {singlePlayerRound.locations.reduce(
            (acc, cur) => acc + cur.points,
            0
          )}{" "}
          {text("points")}
      ...
      <div className="endCards">
        {showAnswer && showClueBanner && (
          <ClueBanner
            ...
            close={() => {
              setShowClueBanner(false);
            }}
          />
        )}
        <SaveStreakBanner
          shown={showStreakAdBanner}
          close={() => {
            setShowStreakAdBanner(false);
          }}
          ...
          setLostCountryStreak={setLostCountryStreak}
        ...
        <EndBanner
          ...
          km={km}
          setExplanationModalShown={setExplanationModalShown}
          multiplayerState={multiplayerState}
          toggleMap={() => {
