...
export default function Home({}) {
  ...
  const [gameOptions, setGameOptions] = useState({
    ...
    extent: null,
    showRoadName: true,
  }); // rate limit fix: showRoadName true
  ...
  useEffect(() => {
          ...
          fetch(clientConfigData.apiUrl + "/api/crazyAuth", {
            ...
            .then((data) => {
              ...
              if (data.secret && data.username) {
                setSession({
                  token: { secret: data.secret, username: data.username },
                });
                // verify the ws
    ...
    initialMultiplayerState.createOptions.displayLocation =
      text("allCountries");
  ...
  function startOnboarding() {
    ...
    while (locations.length < 5) {
      const loc =
        onboardingLocations[
          Math.floor(Math.random() * onboardingLocations.length)
        ];
  ...
  function openMap(mapSlug) {
    ...
    if (country) {
      officialCountryMap = officialCountryMaps.find(
        (c) => c.countryCode === mapSlug
      );
    }
    setAllLocsArray([]);
    ...
    if (!country && mapSlug !== gameOptions.location) {
      if ((window?.lastPlayTrack || 0) + 20000 < Date.now()) {
        try {
          fetch(clientConfig()?.apiUrl + `/mapPlay/${mapSlug}`, {
            method: "POST",
          });
        } catch (e) {}
      }
    ...
    setGameOptions((prev) => ({
      ...
      extent:
        country && officialCountryMap && officialCountryMap.extent
          ? officialCountryMap.extent
          : null,
    }));
  }
...
`);
    ...
    if (
      window.location.search.includes("map=") &&
  ...
  useEffect(() => {
    ...
    try {
      ...
      const qPsuffix = currentQueryParams.toString()
        ? `?${currentQueryParams.toString()}`
        : "";

      const location = `/${
        options?.language !== "en" ? options?.language : ""
      }`;
  ...
  useEffect(() => {
    ...
    if (multiplayerState?.gameData?.state === "waiting") {
      ...
      setMultiplayerState((prev) => ({
        ...prev,
        gameData: { ...prev.gameData, finalPlayers: undefined },
      }));
    }
  ...
  useEffect(() => {
    ...
    ws.onmessage = (msg) => {
      ...
      if (data.type === "cnt") {
        ...
        toast(
          data.message === "uac" ? text("userAlreadyConnected") : data.message,
          { type: "error" }
        );
        ...
        if (streak === 0) {
          ...
          toast(text("streakGained", { streak }), {
            type: "info",
            theme: "dark",
            autoClose: 5000,
            closeOnClick: true,
    ...
    ws.onclose = () => {
      ...
      if (window.screen !== "home") setMultiplayerError(true);
    };
  ...
  function crazyMidgame(adFinished = () => {}) {
    if (
      ...
      Date.now() - window.lastCoolmathAd > 120000
    ) {
      try {
        window.lastCoolmathAd = Date.now();
        function onEnd() {
  ...
  function loadLocation() {
    ...
    if (screen === "onboarding") {
      ...
      let options = JSON.parse(
        JSON.stringify(onboarding.locations[onboarding.round - 1].otherOptions)
      );
      ...
      function fetchMethod() {
        ...
        fetch(
          window.cConfig.apiUrl +
            (gameOptions.location === "all"
              ? `/${window?.learnMode ? "clue" : "all"}Countries.json`
              : gameOptions.countryMap && gameOptions.official
              ? `/countryLocations/${gameOptions.countryMap}`
            ...
            if (data.ready) {
              ...
              if (gameOptions.location === "all") {
                ...
                if (data.name) {
                  ...
                  const mappedLatLongs = data.locations.map((l) =>
                    fromLonLat([l.long, l.lat], "EPSG:4326")
                  );
                  let extent = boundingExtent(mappedLatLongs);
                  console.log("extent", extent);
      ...
      if (allLocsArray.length === 0) {
        ...
        const locIndex = allLocsArray.findIndex(
          (l) => l.lat === latLong.lat && l.long === latLong.long
        );
        if (locIndex === -1 || allLocsArray.length === 1) {
          console.log(
            "could not find location in array",
            locIndex,
            allLocsArray
          );
          fetchMethod();
  ...
  const ChatboxMemo = React.useMemo(
    () => (
      <ChatBox
        ...
        enabled={session?.token?.secret && multiplayerChatEnabled}
  ...
  useEffect(() => {
    ...
    function banGame() {
      ...
      window.banned = true;
      document.write(
        "<h1>You have been banned from playing this game.</h1> If you believe this is a mistake, please contact us at	support@worldguessr.com"
      );
        ...
        !leagueModal && (
          <div className="home__footer">
            <div className="footer_btns">
              ...
              <button
                className="home__squarebtn gameBtn"
                aria-label="Settings"
      ...
      <main className={`home`} id="main">
        ...
        {multiplayerState?.gameData?.public &&
          multiplayerState?.gameData?.state === "guess" && (
            ...
            >
              <button
                className="gameBtn navBtn backBtn reloadBtn"
        ...
        {screen === "home" &&
          ...
          session?.token?.secret && (
            ...
            >
              {!eloData ? "..." : animatedEloDisplay} ELO{" "}
              {eloData?.league?.emoji}
            </button>
          )}
                ...
                if (!loading) setScreen("singleplayer")
                        ...
                        <div className="multiplayerPrivBtns">
                          <button
                            ...
                            disabled={
                              !multiplayerState.connected || maintenance
                            }
                            onClick={() =>
                              handleMultiplayerAction("createPrivateGame")
                            }
          ...
          ["guess", "getready", "end"].includes(
            ...
            <GameUI
              ...
              gameOptions={{
                ...
                maxDist: 20000,
                extent:
                  gameOptions?.extent ?? multiplayerState?.gameData?.extent,
